...
// 整数の N ビットずつを使ってデータを保存する
// アクセスは配列より遅いだろうが、
// パラレルに処理出来るので速くなる部分もあると期待
// TODO: atomic型を用いた時の処理をちゃんとやる
// TODO: operator <<= のオーバーロード以降Xcodeの構文解析が失敗する

template<typename data_t, int MAX_SIZE_, int N_, int ARG_SIZE_, typename atomic_data_t = data_t>
class alignas(data_t) BitArrayInRegister{

public:
    
    using size_t = std::size_t; // unsigned int;
    using entry_t = data_t;
    using this_t = BitArrayInRegister<data_t, MAX_SIZE_, N_, ARG_SIZE_, atomic_data_t>;
    ...
    constexpr BitArrayInRegister(const BitArrayInRegister<data_t, MAX_SIZE_, N_, ARG_SIZE_, atomic_data_t>& arg)
    : dat_(arg.data()){}
    ...
    constexpr data_t any()const noexcept{ return static_cast<data_t>(dat_); }
    ...
    this_t& fill(const entry_t entry)noexcept{ // 同じ値を範囲内に詰める
        dat_ = fillBits<data_t, N_>(entry) & full_mask(); return *this;
    }
    this_t& fill_all(const entry_t entry)noexcept{ // 同じ値を範囲外まで詰める
        dat_ = fillBits<data_t, N_>(entry); return *this;
    }

    this_t& set_sequence()noexcept{ // TODO:コンパイル時計算されてる?
        clear();
    ...
    constexpr data_t data()const noexcept{ return static_cast<data_t>(dat_); }
    atomic_data_t& data()noexcept{ return dat_; }
    ...
    this_t& set(const size_t n, const entry_t entry)noexcept{
        assert_index(n);
        dat_ |= (entry << place(n));
        return *this;
    }
    this_t& replace(const size_t n, const entry_t entry)noexcept{
        assert_index(n);
        dat_ = (dat_ & (~mask(n))) | (entry << place(n));
        return *this;
    }
    this_t& replace(const size_t n, const entry_t entry, const size_t l)noexcept{ // 長さ|l|分のデータを入れ替え
        assert_index(n);
        data_t msk = lower_mask(l) << place(n);
        dat_ = (dat_ & (~msk)) | (entry << place(n));
        return *this;
    }
    this_t& clear(const size_t n)noexcept{
        assert_index(n);
        dat_ &= ~mask(n);
        return *this;
    }
    this_t& assign(const size_t n, const entry_t entry)noexcept{
        replace(n, entry);
        return *this;
    }
    this_t& assign(const size_t n, const entry_t entry, const size_t l)noexcept{
        replace(n, entry, l);
        return *this;
    }
    this_t& set_flag(const size_t n, const size_t fn)noexcept{
        assert_index(n);
        dat_ |= (MASK_BIT_ << (place(n) + fn));
        return *this;
    }
        
    this_t& set_by_bit_index(const size_t n, const entry_t entry)noexcept{
        dat_ |= (entry << n);
        return *this;
    }
    this_t& assign_by_bit_index(const size_t n, const entry_t entry)noexcept{
    ...
    this_t& set_part(const size_t n, const BitArrayInRegister<data_t, MAX_SIZE_, N_A, SIZE_A, other_atomic_data_t> arg)noexcept{
        assert_index(n);
        dat_ |= arg.get_part(n);
        return *this;
    }
    template<int N_A, int SIZE_A, typename other_atomic_data_t>
    this_t& replace_part(const size_t n, const BitArrayInRegister<data_t, MAX_SIZE_, N_A, SIZE_A, other_atomic_data_t> arg)noexcept{
        assert_index(n);
        dat_ = (dat_ & (~mask(n))) | (arg.data() & mask(n));
        return *this;
    }
    template<int N_A, int SIZE_A, typename other_atomic_data_t>
    this_t& assign_part(const size_t n, const BitArrayInRegister<data_t, MAX_SIZE_, N_A, SIZE_A, other_atomic_data_t> arg)noexcept{
        return replace_part<N_A, SIZE_A>(n, arg);
    }

    // 回転
    this_t& rotate_front(const size_t n)noexcept{
        assert_index(n);
        dat_ = ((dat_ & lower_mask(n)) << ((N_ * SIZE_) - place(n)))
            | ((dat_ & (~lower_mask(n))) >> place(n));
        return *this;
    }
    this_t& rotate_back(const size_t n)noexcept{
        return rotate_front(SIZE_ - n);
    }
    this_t& rotate(const size_t n)noexcept{
        return rotate_front(n);
    }

    // 加減算
    this_t& plus(const size_t n, const entry_t entry)noexcept{
        dat_ += (entry << place(n));
        return *this;
    }
    this_t& minus(const size_t n, const entry_t entry)noexcept{
        dat_ -= (entry << place(n));
        return *this;
    }
    
    this_t& add(const size_t n, const entry_t entry)noexcept{
        dat_ += (entry << place(n));
        return *this;
    }
    this_t& subtr(const size_t n, const entry_t entry)noexcept{
        dat_ -= (entry << place(n));
        return *this;
    }

    // データ押し込み
    this_t& insert_push_into_back(const size_t n, const entry_t entry)noexcept{
        assert_index(n);
        assert_entry(entry);
    ...
    this_t& insert_push_into_front(const size_t n, const entry_t entry)noexcept{
        assert_index(n);
        assert_entry(entry);
    ...
    this_t& insert(const size_t n, const entry_t entry)noexcept{
        return insert_push_into_back(n, entry);
    }

    // ある位置のものを抜いて、1つ詰める
    this_t& remove_front()noexcept{
        dat_ >>= N_; return *this;
    }
    this_t& remove_back()noexcept{
        dat_ &= (~mask(SIZE_ - 1)); return *this;
    }
    this_t& remove_pull_back(const size_t n)noexcept{
        assert_index(n);
        dat_ = (dat_ & mask(0, n - 1)) | ((dat_ >> N_) & (~mask(0, n - 1)));
        return *this;
    }
    this_t& remove_pull_front(const size_t n)noexcept{
        assert_index(n);
        dat_ = ((dat_ << N_) & mask(0, n - 1)) | (dat_ & (~mask(0, n - 1)));
        return *this;
    }

    this_t& remove(const size_t n)noexcept{ return remove_pull_back(n); }

    // スワップ
    this_t& swap(const size_t n0, const size_t n1)noexcept{
